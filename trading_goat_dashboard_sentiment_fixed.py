...
<style>
    /* =========================================================
       THEME-SAFE UI: readable in Streamlit light OR dark mode
       Force an app-level institutional dark theme so Streamlit's
       light mode does not create low-contrast text/cards.
       ========================================================= */
    ...
    html, body, [data-testid="stAppViewContainer"], .stApp {
        background: radial-gradient(circle at top left, #1e1b4b 0%, #0f172a 38%, #020617 100%) !important;
    ...
    /* Global readable text colors */
    p, span, div, label, small, li, strong, em,
    ...
    /* Data editor / dataframe readability */
    [data-testid="stDataFrame"] * {
        color: var(--text-main) !important;
    }

    /* Download buttons / forms */
...
with st.sidebar:
    ...
    ticker = st.text_input("Ticker", "AAPL", key="main_ticker_input", help="Enter stock or ETF symbol", label_visibility="collapsed").strip().upper()
    ...
    data_provider = st.radio(
        "Market Data Source",
        ["Auto", "Polygon.io", "Yahoo Finance"],
        index=0,
        help="Auto uses Polygon when an API key is provided, otherwise Yahoo/Stooq fallback."
    )
    polygon_api_key = st.text_input(
        "Polygon.io API Key",
        type="password",
        help="Optional. Improves reliability and can reduce Yahoo Finance rate-limit issues."
    )
    use_polygon = (
        (data_provider == "Polygon.io" and polygon_api_key.strip() != "")
        or (data_provider == "Auto" and polygon_api_key.strip() != "")
    )
    if data_provider == "Polygon.io" and not polygon_api_key.strip():
        st.warning("Enter a Polygon.io API key or switch to Auto/Yahoo.")
    active_data_provider = "Polygon.io" if use_polygon else "Yahoo/Stooq fallback"
    st.caption(f"Active source: {active_data_provider}")

    st.markdown("---")
    
    st.markdown("### 📊 Chart Settings")
    period = st.selectbox("Time Period", ["5d", "1mo", "3mo", "6mo", "1y", "2y", "5y"], index=3, help="Historical data range")
    interval = st.selectbox("Interval", ["5m", "15m", "30m", "1h", "1d", "1wk", "1mo"], index=4, help="Data granularity")

    supports_extended = interval in ["1m", "2m", "5m", "15m", "30m", "60m", "90m", "1h"]
    use_extended_hours = st.toggle(
        "Extended Hours",
        value=False,
        disabled=not supports_extended,
        help="Include pre-market and after-hours data when supported. Only applies to intraday intervals."
    )
    if not supports_extended:
        use_extended_hours = False
        st.caption("Extended hours is only available for intraday intervals like 5m, 15m, 30m, or 1h.")
    elif use_extended_hours:
        st.caption("Including pre-market and after-hours data when the selected provider supports it.")
...
def normalize_ticker_for_yahoo(ticker: str) -> str:
    """Normalize common Canadian/TSX ticker formats for yfinance/Yahoo."""
...
def normalize_ticker_for_polygon(ticker: str) -> str:
    """Best-effort symbol normalization for Polygon. US tickers work directly; TSX via Yahoo fallback."""
    t = str(ticker).strip().upper()
    # Polygon stocks endpoint typically expects US-listed tickers. Keep TSX/yfinance-style tickers for fallback.
    if t.endswith(".TO") or t.endswith(".V") or t in {"HPS.A", "HPS-A", "HPS-A.TO"}:
        return ""
    return t.replace("-", ".")

@st.cache_data(ttl=300, show_spinner=False)
def fetch_polygon_data(ticker, period, interval, api_key, include_extended=False):
    """Fetch OHLCV bars from Polygon.io Aggregates endpoint. Returns empty df on unsupported symbols/errors."""
    ...
    url = (
        ...
        f"?adjusted=true&sort=asc&limit=50000{'&extended_hours=true' if include_extended and timespan in ['minute', 'hour'] else ''}&apiKey={key}"
    )
    try:
        ...
        out["Datetime"] = pd.to_datetime(out["t"], unit="ms", utc=True).dt.tz_convert("America/New_York")
...
def fetch_data(ticker, period, interval, use_polygon=False, polygon_api_key="", include_extended=False):
    """
    Rate-limit-safe market data fetcher.

    Important changes:
    - Uses yf.download() instead of yf.Ticker().history() to avoid extra quote/info calls.
    - Uses threads=False to reduce Yahoo rate-limit bursts.
    - Does NOT call .info by default; fundamentals are fetched separately only when needed.
    - Falls back to Stooq daily data for common US tickers when Yahoo is temporarily blocked.
    """
    ...
    # Polygon primary source when selected/key is present.
    if use_polygon:
        df = fetch_polygon_data(ticker, period, interval, polygon_api_key, include_extended)
        if not df.empty:
            if include_extended and interval in ["1m", "2m", "5m", "15m", "30m", "60m", "90m", "1h"]:
                try:
                    idx = pd.to_datetime(df.index)
                    idx_ny = idx.tz_convert("America/New_York") if getattr(idx, 'tz', None) is not None else idx
                    minutes = idx_ny.hour * 60 + idx_ny.minute
                    regular = (minutes >= 570) & (minutes < 960)
                    df["Session"] = np.where(regular, "Regular", "Extended")
                except Exception:
                    df["Session"] = "Unknown"
            else:
                df["Session"] = "Regular"
            ...
            return df, {"data_source": "Polygon.io" + (" + extended hours" if include_extended else "")}
        # If Polygon doesn't support the symbol (ex: TSX), fall back silently to Yahoo/Stooq.
    ...
    try:
        ...
        if 'Too Many Requests' in msg or 'Rate limited' in msg or '429' in msg:
            st.warning(
                ...
                "Using cache/fallback where possible. Try Low API Mode or wait a few minutes."
            )
        else:
            st.warning(f"Temporary data issue for {ticker}: {msg}")

    # Stooq fallback: daily US symbols only. This keeps AAPL/MSFT/SPY usable when Yahoo blocks.
    if df.empty and interval in ['1d', '1wk'] and yahoo_ticker.replace('.', '').replace('-', '').isalpha() and not yahoo_ticker.endswith('.TO'):
        df = fetch_stooq_daily(yahoo_ticker, period)
    ...
    # Label regular vs extended-hours bars when using intraday data.
    # This is useful for separating overnight/pre-market flow from regular-session flow.
    if include_extended and interval in ["1m", "2m", "5m", "15m", "30m", "60m", "90m", "1h"]:
        try:
            idx = pd.to_datetime(df.index)
            # Convert to New York time when timezone-aware; otherwise assume local exchange-like times.
            if getattr(idx, 'tz', None) is not None:
                idx_ny = idx.tz_convert("America/New_York")
            else:
                idx_ny = idx
            minutes = idx_ny.hour * 60 + idx_ny.minute
            regular = (minutes >= 570) & (minutes < 960)  # 9:30 to 16:00 ET
            df["Session"] = np.where(regular, "Regular", "Extended")
        except Exception:
            df["Session"] = "Unknown"
    else:
    ...
    # Keep info empty here to avoid Yahoo quoteSummary rate limits.
    return df, {}

@st.cache_data(ttl=3600, show_spinner=False)
def fetch_stooq_daily(ticker, period='1y'):
    """Fallback daily data from Stooq for US tickers when yfinance is rate-limited."""
    try:
        ...
        out = pd.read_csv(StringIO(resp.text), parse_dates=['Date'])
        if out.empty:
            return pd.DataFrame()
        out = out.rename(columns={'Date': 'Datetime'}).set_index('Datetime')
        out = out[['Open', 'High', 'Low', 'Close', 'Volume']].dropna()

        # Apply rough period trimming
        days_map = {'5d': 7, '1mo': 35, '3mo': 100, '6mo': 190, '1y': 370, '2y': 740, '5y': 1900}
...
def fetch_company_info(ticker):
    """Optional company/fundamental info. Cached for 6h and skipped if Yahoo blocks."""
    if not YF_AVAILABLE:
        return {}
    try:
...
def calculate_advanced_metrics(df, info):
    ...
    # Beta (compared to SPY) - uses cached safe fetcher, not raw yf.download
    try:
        spy_data, _ = fetch_data('SPY', period='1y', interval='1d')
...
@st.cache_data(ttl=900, show_spinner=False)
def fetch_news_articles(ticker, max_age_days=3, sentiment_enabled=True):
    """Fetch recent ticker news.

    When sentiment_enabled=False, headlines still load, but sentiment and
    impact scores are neutral. This makes the sidebar toggle actually change
    what the dashboard computes/displays.
    """
    if not FEEDPARSER_AVAILABLE:
        return []

    def clean_symbol(sym):
        return str(sym).upper().replace('.TO', '').replace('-A', '.A').strip()
    ...
    def score_news_title(title):
        ...
        # Phrase rules first: avoids false positives from isolated words.
        bullish_phrases = [
            r"beat(s|ing)? .*expect", r"raises? .*guidance", r"record .*revenue",
            r"wins? .*contract", r"announces? .*buyback", r"approved by",
            r"strategic partnership", r"acquisition .*completed", r"shares? (surge|jump|soar|rally|gain)"
        ]
        bearish_phrases = [
            r"miss(es|ed)? .*expect", r"cuts? .*guidance", r"downgrade",
            r"investigation", r"lawsuit", r"bankruptcy", r"delisting",
            r"shares? (plunge|drop|fall|crash|tumble|sink)"
        ]
        import re as _re
        score = 0
        if any(_re.search(p, txt) for p in bullish_phrases):
            score += 2
        if any(_re.search(p, txt) for p in bearish_phrases):
            score -= 2

        # Lighter keyword fallback.
        bullish = ['surge','jump','soar','rally','gain','beat','upgrade','buyback','contract','approval','record']
        bearish = ['plunge','drop','fall','crash','tumble','sink','miss','downgrade','lawsuit','probe','warning']
        score += sum(1 for w in bullish if w in txt)
        score -= sum(1 for w in bearish if w in txt)
        score = max(-3, min(3, score))

        high_impact_terms = [
            'earnings','guidance','acquisition','merger','contract','deal','fda',
            'approval','lawsuit','investigation','buyback','dividend','ceo','cfo',
            'bankruptcy','offering','financing','partnership'
        ]
        impact = 30 + 15 * abs(score) + (25 if any(w in txt for w in high_impact_terms) else 0)
        return score, min(100, impact)

    def detect_press_release(title, source):
        txt = title.lower()
        src = source.lower()
        pr_sources = ['pr newswire','prnewswire','business wire','businesswire','globenewswire','accesswire','newsfile']
        pr_patterns = ['announces','reports fiscal','reports q', 'declares dividend', 'appoints', 'acquires', 'partnership']
        return any(s in src for s in pr_sources) or any(p in txt for p in pr_patterns)

    symbol = clean_symbol(ticker)
    query_symbol = ticker.replace('.TO','').replace('-A','.A')
    articles, seen = [], set()
    feeds = [
        ...
        (f"https://news.google.com/rss/search?q={query_symbol}+stock+when:1d&hl=en-US&gl=US&ceid=US:en", 'Google News'),
        (f"https://news.google.com/rss/search?q={query_symbol}+PR+Newswire+when:1d&hl=en-US&gl=US&ceid=US:en", 'PR Newswire / Google'),
        (f"https://news.google.com/rss/search?q={query_symbol}+Business+Wire+when:1d&hl=en-US&gl=US&ceid=US:en", 'Business Wire / Google'),
        (f"https://news.google.com/rss/search?q={query_symbol}+GlobeNewswire+when:1d&hl=en-US&gl=US&ceid=US:en", 'GlobeNewswire / Google'),
        ("https://www.prnewswire.com/rss/news-releases-list.rss", 'PR Newswire'),
        ("https://www.businesswire.com/rss/home/?rss=G1QFDERJXkJeEFJQXw==", 'Business Wire'),
        ("https://www.globenewswire.com/RssFeed/subjectcode/1", 'GlobeNewswire'),
    ]

    for feed_url, default_source in feeds:
        try:
            feed = feedparser.parse(feed_url)
            source_name = feed.feed.get('title', default_source) if hasattr(feed, 'feed') else default_source
            for entry in feed.entries[:30]:
                ...
                # Broad feeds must mention the ticker/company symbol somewhere.
                if default_source in ['PR Newswire','Business Wire','GlobeNewswire'] and symbol not in title_upper and query_symbol.upper() not in title_upper:
                    continue
                ...
                age_hrs = (datetime.utcnow() - pub_dt).total_seconds() / 3600
                if age_hrs < 0 or age_hrs > max_age_days * 24:
                    continue

                sentiment, impact_score = score_news_title(title_raw)
                is_press = detect_press_release(title_raw, source_name)
                txt = title_raw.lower()
                is_high_impact = impact_score >= 70 or any(w in txt for w in ['earnings','guidance','acquisition','merger','fda','approval','contract','lawsuit','investigation'])
                seen.add(title_raw)
                articles.append({
                    'title': title_raw,
                    'url': entry.get('link', ''),
                    ...
                    'age_hrs': age_hrs,
                    'sentiment': sentiment,
                    'impact_score': impact_score if sentiment_enabled else 0,
                    'is_press': is_press,
                    'is_high_impact': is_high_impact if sentiment_enabled else False
                })
        except Exception:
            continue

    return sorted(articles, key=lambda x: (x.get('age_hrs', 999), -x.get('impact_score', 0)))[:50]


def summarize_news_sentiment(articles):
    """Return a compact news sentiment/impact summary for the current ticker."""
    if not articles:
        return {'score': 0, 'impact': 0, 'label': 'No fresh news', 'bullish': 0, 'bearish': 0, 'neutral': 0}
    sentiments = [a.get('sentiment', 0) for a in articles]
    impacts = [a.get('impact_score', 0) for a in articles]
    avg_sentiment = sum(sentiments) / max(1, len(sentiments))
    avg_impact = sum(impacts) / max(1, len(impacts))
    if avg_sentiment > 0.35:
        label = '🟢 Bullish news tone'
    elif avg_sentiment < -0.35:
        label = '🔴 Bearish news tone'
    else:
        label = '⚪ Neutral / mixed news tone'
    return {
        'score': round(avg_sentiment, 2),
        'impact': round(avg_impact, 0),
        'label': label,
        'bullish': sum(1 for s in sentiments if s > 0),
        'bearish': sum(1 for s in sentiments if s < 0),
        'neutral': sum(1 for s in sentiments if s == 0),
    }


def apply_news_sentiment_to_score(score_dict, articles, sentiment_enabled=True):
    """Adjust composite buy/sell score with recent news only when toggle is ON."""
    out = dict(score_dict)
    out.setdefault('reasons_buy', [])
    out.setdefault('reasons_sell', [])
    out['sentiment_adjustment'] = 0
    if not sentiment_enabled or not articles:
        return out
    summary = summarize_news_sentiment(articles)
    # Max 12 pts; high-impact headlines matter more than low-impact headlines.
    pts = int(min(12, abs(summary['score']) * 6 + summary['impact'] / 20))
    if summary['score'] > 0.25:
        out['buy'] = min(100, out.get('buy', 0) + pts)
        out['reasons_buy'] = list(out['reasons_buy']) + [f"Recent bullish news sentiment +{pts}"]
        out['sentiment_adjustment'] = pts
    elif summary['score'] < -0.25:
        out['sell'] = min(100, out.get('sell', 0) + pts)
        out['reasons_sell'] = list(out['reasons_sell']) + [f"Recent bearish news sentiment +{pts}"]
        out['sentiment_adjustment'] = -pts
    out['gap'] = abs(out.get('buy', 0) - out.get('sell', 0))
    if out['buy'] >= 70 and out['buy'] > out['sell']:
        out['signal'] = '🟢 BUY / ACCUMULATION'
    elif out['sell'] >= 70 and out['sell'] > out['buy']:
        out['signal'] = '🔴 SELL / DISTRIBUTION'
    elif out['gap'] < 15:
        out['signal'] = '⚪ NO EDGE'
    else:
        out['signal'] = '🟡 WATCH'
...
def compute_buy_sell_score(frame: pd.DataFrame, ml_prediction=None, ml_probability=None, use_ml=True) -> dict:
    ...
    if frame is None or frame.empty:
        return {'buy': 0, 'sell': 0, 'gap': 0, 'signal': 'No data', 'reasons_buy': [], 'reasons_sell': [], 'ml_adjustment': 0, 'sentiment_adjustment': 0}
    row = frame.iloc[-1]
    buy = sell = 0
    rb, rs = [], []
...
if active_tab in MAIN_DATA_PAGES:
    ...
    with st.spinner(f"🔄 Loading price data for {ticker}..."):
        df, info = fetch_data(ticker, period, interval, use_polygon, polygon_api_key, use_extended_hours)

    if df.empty:
        st.error(
            f"❌ Unable to fetch price data for {ticker}. "
            "Yahoo may be rate-limiting the app. Try a daily interval, wait a few minutes, or use a smaller active page scan."
        )
        st.stop()

    if isinstance(info, dict) and info.get("data_source"):
        st.caption(f"Data source: {info.get('data_source')}")
    elif use_polygon:
        st.caption("Data source: Yahoo/Stooq fallback (Polygon unavailable for this symbol or request)")

    # Fetch heavy company info only where it is actually useful.
    # This avoids rate limits on pages that only need price/volume data.
    if active_tab == "📊 Analysis":
        info = fetch_company_info(normalize_ticker_for_yahoo(ticker))
    else:
        info = {}
    ...
    # Sentiment toggle now actually changes the composite score.
    # Kept to 1-day lookback to avoid stale headlines and unnecessary API/RSS work.
    sentiment_articles_for_score = []
    if enable_sentiment_analysis:
        sentiment_articles_for_score = fetch_news_articles(ticker, max_age_days=1, sentiment_enabled=True)
        combined_score = apply_news_sentiment_to_score(combined_score, sentiment_articles_for_score, sentiment_enabled=True)
    ...
    st.markdown("---")
    st.markdown("### 🧭 Composite Signal — Technical + Flow" + (" + ML" if enable_ml_prediction else "") + (" + News Sentiment" if enable_sentiment_analysis else ""))
    ...
    if enable_sentiment_analysis:
        st.caption(f"News sentiment adjustment applied: {combined_score.get('sentiment_adjustment', 0):+d} points from fresh headlines.")
    else:
        st.caption("Sentiment Analysis is OFF: news headlines are displayed neutrally and do not affect the composite score.")
...
if active_tab == "📊 Analysis":
    ...
    with col2:
        st.plotly_chart(build_volume_profile(df), width="stretch")
    fig_bs = go.Figure()
    ...
    if not sector_df.empty:
        ...
        with left:
            ...
            st.dataframe(sector_df.head(8), width="stretch", hide_index=True)
        with right:
            st.markdown("#### Top outflows")
            st.dataframe(sector_df.tail(8).sort_values('Rotation Score'), width="stretch", hide_index=True)
        st.markdown("#### 🗺️ Rotation Strength Map")
        map_df = sector_df.copy()
        map_df['Rotation Strength'] = map_df['Rotation Score'].clip(1, 100)
        map_df['Path Root'] = 'Market Map'

        # Build the treemap with a proper root -> group -> sector/theme hierarchy.
        # This is more reliable on Streamlit Cloud than a parent list without root nodes.
        try:
            import plotly.express as px
            fig_heat = px.treemap(
                map_df,
                path=['Path Root', 'Group', 'Name'],
                values='Rotation Strength',
                ...
                hover_data={
                    ...
                    'Signal': True,
                    'Path Root': False,
                    'Rotation Strength': False,
                }
            )
            fig_heat.update_layout(
                template='plotly_dark',
                height=620,
                title='Rotation Strength Map — green = inflow / red = outflow',
                margin=dict(t=50, l=10, r=10, b=10),
            )
            fig_heat.update_traces(textinfo='label+value')
            st.plotly_chart(fig_heat, width='stretch')
        except Exception as e:
            st.warning(f"Treemap could not render, showing heatmap fallback instead: {e}")
            fig_fallback = go.Figure(go.Heatmap(
                z=[map_df['Rotation Score'].tolist()],
                x=map_df['Name'].tolist(),
                y=['Rotation'],
                colorscale='RdYlGn',
                zmin=0,
                zmax=100,
                text=[[f"{n}<br>{s:.0f}" for n, s in zip(map_df['Name'], map_df['Rotation Score'])]],
                texttemplate='%{text}',
                hovertemplate='%{x}<br>Rotation Score: %{z:.0f}<extra></extra>',
            ))
            fig_fallback.update_layout(template='plotly_dark', height=320, title='Rotation Strength Heatmap Fallback')
            st.plotly_chart(fig_fallback, width='stretch')

        fig_bar = go.Figure(go.Bar(
            x=sector_df['Name'],
            y=sector_df['Rel vs SPY %'],
            marker_color=np.where(sector_df['Rel vs SPY %'] >= 0, '#22c55e', '#ef4444')
        ))
    ...
    if st.button("🔍 Scan Watchlist"):
        ...
        if results:
            st.dataframe(pd.DataFrame(results).sort_values(['Buy','Gap'], ascending=False), width="stretch", hide_index=True)

elif active_tab == "📰 News":
    st.markdown("### 📰 News & Sentiment")
    news_days = st.slider("News lookback days", 1, 30, 1)
    st.caption("Sentiment Analysis toggle is in the sidebar under 🤖 AI Features.")

    articles = fetch_news_articles(ticker, max_age_days=news_days, sentiment_enabled=enable_sentiment_analysis)

    if articles:
        impact_count = sum(1 for a in articles if a.get('is_high_impact'))
        press_count = sum(1 for a in articles if a.get('is_press'))
        summary = summarize_news_sentiment(articles) if enable_sentiment_analysis else {'label': 'Sentiment disabled', 'score': 0, 'impact': 0}
        n1, n2, n3, n4 = st.columns(4)
        n1.metric("Headlines", len(articles))
        n2.metric("High impact", impact_count if enable_sentiment_analysis else "Off")
        n3.metric("Press releases", press_count)
        n4.metric("News tone", summary['label'], help="Only active when Sentiment Analysis is checked.")

        if enable_sentiment_analysis:
            st.info(f"Sentiment is ON: average score {summary['score']:+.2f}, average impact {summary['impact']:.0f}/100. These fresh headlines can adjust the composite signal.")
        else:
            st.warning("Sentiment is OFF: headlines are shown as neutral and do not affect buy/sell score, impact score, or alerts.")

        rows = []
        for article in articles[:30]:
            sentiment_icon = "🟢" if article.get('sentiment', 0) > 0 else "🔴" if article.get('sentiment', 0) < 0 else "⚪"
            ...
            extra = f"Impact: {article.get('impact_score', 0)}/100 • Sentiment: {article.get('sentiment', 0):+d} • " if enable_sentiment_analysis else ""
            st.caption(f"{extra}{article['source']} • {article['age_hrs']:.0f}h ago")
            rows.append({
                'Title': article['title'],
                'Source': article['source'],
                'Age (hrs)': round(article['age_hrs'], 1),
                'Sentiment': article.get('sentiment', 0) if enable_sentiment_analysis else 0,
                'Impact': article.get('impact_score', 0) if enable_sentiment_analysis else 0,
                'Press Release': article.get('is_press', False),
                'High Impact': article.get('is_high_impact', False) if enable_sentiment_analysis else False,
            })
        with st.expander("News table"):
