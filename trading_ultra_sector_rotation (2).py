...
st.caption("VWAP · RSI · MACD · OBV · Whale Flow · Dark-Pool Proxy · Watchlist Scanner · Hidden Gem Scanner · Sector Heatmap · Real Alerts · Portfolio Tracker")
...
with st.sidebar:
    ...
    enable_real_alerts = st.toggle("Enable real alerts", value=False, help="Sends webhook messages when fresh high-conviction signals appear.")
...
# =========================================================
# SECTOR ROTATION HELPERS — FULL 11 GICS SECTORS
# =========================================================
@st.cache_data(ttl=300)
def scan_sector_rotation(period: str = "3mo", interval: str = "1d") -> pd.DataFrame:
    """Scan the 11 GICS sector ETF proxies and rank sector inflow/outflow."""
    rows = []
    for sector_name, etf in GICS_SECTOR_ETFS.items():
        ...
        # Rotation score mixes current signal, price trend, OBV direction, and accumulation clusters.
        rotation_score = (
            net_flow * 0.45
            + np.clip(perf_lookback, -20, 20) * 1.25
...
def build_sector_rotation_timeline(period: str = "6mo", interval: str = "1d") -> pd.DataFrame:
    ...
    for sector_name, etf in GICS_SECTOR_ETFS.items():
        ...
        tmp = pd.DataFrame({"Date": joined.index, "Sector": sector_name, "ETF": etf, "RelativeStrength": rel_strength.values})
...
# TABS
# =========================================================
tab_overview, tab_price, tab_volume, tab_indicators, tab_watchlist, tab_hidden, tab_sector, tab_portfolio, tab_alerts, tab_news, tab_backtest, tab_settings = st.tabs([
    "Overview",
    "Price Chart",
    "Whale Flow",
    "Indicators",
...
with tab_sector:
    st.subheader("Sector Rotation Dashboard — 11 GICS Sectors")
    st.caption("Uses sector ETF proxies: XLK, XLV, XLF, XLY, XLP, XLI, XLE, XLB, XLU, XLRE, XLC.")
    ...
    if sector_df.empty:
        ...
        m3.metric("Positive sectors", f"{risk_on_count}/11")

        st.write("### Live sector ranking")
        st.dataframe(
            sector_df[[
                "Rank", "Sector", "ETF", "RotationScore", "NetFlow", "BuyScore", "SellScore",
                "20-Bar Return %", "OBV Direction", "Whale Bars 20", "Accum Signals 20", "Dist Signals 20", "Regime"
            ]],
            use_container_width=True,
            hide_index=True,
        )

        st.write("### Top inflow / outflow sectors")
        ic1, ic2 = st.columns(2)
        with ic1:
            st.success(f"Strongest inflows: {inflow_text}")
            st.dataframe(
                sector_df.sort_values("RotationScore", ascending=False).head(5)[["Sector", "ETF", "RotationScore", "NetFlow", "OBV Direction", "Regime"]],
                use_container_width=True,
                hide_index=True,
            )
        with ic2:
            ...
            st.dataframe(
                sector_df.sort_values("RotationScore", ascending=True).head(5)[["Sector", "ETF", "RotationScore", "NetFlow", "OBV Direction", "Regime"]],
                use_container_width=True,
                hide_index=True,
            )
        ...
        fig_tree = go.Figure(go.Treemap(
            ...
            values=sector_df["RotationScore"].abs() + 5,
            marker=dict(
                colors=sector_df["RotationScore"],
                colorscale="RdYlGn",
                cmid=0,
        ...
        fig_tree.update_layout(height=560, title="Sector Heatmap — Full 11 GICS Sectors")
        ...
        fig_rank.update_layout(height=440, title="Sector Ranking — Rotation Score", yaxis_title="Rotation Score")
        ...
        if timeline_df.empty:
            ...
            # Plot all 11 sectors as relative strength vs SPY.
            ...
            fig_timeline.update_layout(
                height=560,
                title="Sector Rotation Timeline — 20-Bar Relative Strength vs SPY",
        ...
        for sec in sector_df["Sector"]:
            ...
            with st.expander(f"{row['Sector']} — {row['ETF']} — Rotation {row['RotationScore']:+.1f}"):
                ...
                st.write(
                    f"20-bar return: **{row['20-Bar Return %']}%** · OBV: **{row['OBV Direction']}** · "
                    f"Whale bars in last 20: **{row['Whale Bars 20']}** · "
                    f"Accum/dist signals: **{row['Accum Signals 20']} / {row['Dist Signals 20']}**"
                )

        st.caption("Sector ETFs are proxies, not perfect sector ownership maps. Use this to detect rotation, then confirm with individual tickers.")
...
with tab_alerts:
    ...
    st.write(f"Current signal: **{score['signal']}** · Buy {score['buy']} / Sell {score['sell']} · Regime: {regime}")
    if not enable_real_alerts:
