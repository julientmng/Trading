...
with st.sidebar:
    ...
    ticker = st.text_input("", "AAPL", key="main_ticker", help="Enter stock or ETF symbol").strip().upper()
        ...
        for category, tickers in ETF_DISCOVERY_THEMES[theme_choice].items():
            with st.expander(f"📁 {category}"):
                for etf_ticker in tickers[:5]:
                    ...
                    with col3:
                        if st.button(f"Analyze", key=f"analyze_{etf_ticker}"):
                            st.session_state['main_ticker'] = etf_ticker
                            st.rerun()
