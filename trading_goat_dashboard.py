...
with tab_etfs:
    st.subheader("🧭 ETF Discovery + Fee Saver")
    st.caption("Browse themes like the screenshot, then use your sector/theme flow tabs to check whether money is rotating into the area.")

    for section, groups in ETF_DISCOVERY_THEMES.items():
        st.markdown(f"### {section}")
        cols = st.columns(4)
        for i, (theme, etfs) in enumerate(groups.items()):
            with cols[i % 4]:
                st.markdown(f"**{theme}**")
                st.write(", ".join(etfs))
    ...
    st.caption("Enter ETFs you hold. The tool estimates whether a cheaper similar ETF exists in the built-in map.")
    ...
    st.caption("Highest-expense ETFs in the built-in map. High fees are not always bad, but they need to earn their keep.")
