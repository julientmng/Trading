...
def score_headline(text: str) -> int:
    """Simple transparent headline sentiment score."""
    t = text.lower()
    return int(sum(w in t for w in BULLISH_WORDS) - sum(w in t for w in BEARISH_WORDS))

def is_press_release(title: str, source: str = "") -> bool:
    t = f"{title} {source}".lower()
    return any(w in t for w in PRESS_RELEASE_WORDS) or any(s in t for s in PRESS_SOURCE_HINTS)
...
with tab_news:
    ...
    confirmation = whale_news_confirmation(articles, df)
    ...
    if not articles:
        ...
        st.caption(
            "Free RSS feeds can be incomplete or delayed. For professional alerts, wire this section to paid Benzinga, PR Newswire, BusinessWire, or Polygon news APIs."
        )
