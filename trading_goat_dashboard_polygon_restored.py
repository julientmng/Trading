...
def fetch_polygon_data(ticker, period, interval, api_key):
...
def fetch_data(ticker, period, interval, use_polygon=False, polygon_api_key=""):
    """
    ...
    if use_polygon:
        df = fetch_polygon_data(ticker, period, interval, polygon_api_key)
        if not df.empty:
            df['Returns'] = df['Close'].pct_change()
            df['Log_Returns'] = np.log(df['Close'] / df['Close'].shift(1))
            df['Cumulative_Returns'] = (1 + df['Returns'].fillna(0)).cumprod()
            return df, {"data_source": "Polygon.io"}
...
if active_tab in MAIN_DATA_PAGES:
    ...
    with st.spinner(f"🔄 Loading price data for {ticker}..."):
        df, info = fetch_data(ticker, period, interval, use_polygon, polygon_api_key)
