...
st.caption("VWAP · RSI · MACD · OBV · Whale Flow · GOAT Quality Score · ETF Themes · Hidden Gems · Sector Heatmap · Real Alerts · Portfolio Tracker")
...
# TABS
# =========================================================
tab_overview, tab_goat, tab_etfs, tab_price, tab_volume, tab_indicators, tab_watchlist, tab_hidden, tab_sector, tab_portfolio, tab_alerts, tab_news, tab_calendar, tab_backtest, tab_settings = st.tabs([
    "Overview",
    "GOAT Quality",
    "ETF Discovery",
    "Price Chart",
...
with tab_etfs:
    st.subheader("🧭 ETF Discovery Search Engine")
    st.caption("Search ETFs by keyword or combine tags like AI, Defense, Gold, Dividend, Japan, Quality, Low Fee, etc. This is no longer a fixed list.")

    search_cols = st.columns([2, 1, 1])
    with search_cols[0]:
        etf_query = st.text_input("Search ETFs", placeholder="Examples: AI, defense, gold, dividend, Japan, low fee, uranium")
    with search_cols[1]:
        etf_match_mode = st.radio("Tag match", ["Any", "All"], horizontal=True)
    with search_cols[2]:
        max_etf_results = st.slider("Max results", 5, 100, 25)

    filter_cols = st.columns([2, 2])
    with filter_cols[0]:
        selected_categories = st.multiselect("Categories", ETF_CATEGORIES, default=[])
    with filter_cols[1]:
        selected_tags = st.multiselect("Tags", ETF_TAGS, default=[])
    ...
    quick_sections = {
        ...
        "Income": ["Dividend", "REITs", "Bonds", "Treasuries", "Cash"],
        "Geography": ["US Large Cap", "US Small Cap", "Europe", "UK", "Japan", "China", "India", "Emerging Markets", "Canada"],
        "Style": ["Quality", "Momentum", "Value", "Low Fee"],
    }

    # Streamlit buttons can't directly mutate a multiselect cleanly without a rerun,
    # so these buttons write selected quick tags to session_state.
    if "etf_quick_tags" not in st.session_state:
        st.session_state.etf_quick_tags = []
    ...
    for section, tags in quick_sections.items():
        ...
        for i, tag in enumerate(tags):
            with cols[i % 6]:
                if st.button(tag, key=f"etf_tag_{section}_{tag}"):
                    if tag not in st.session_state.etf_quick_tags:
                        st.session_state.etf_quick_tags.append(tag)
                    st.rerun()

    active_tags = list(dict.fromkeys((selected_tags or []) + st.session_state.get("etf_quick_tags", [])))
    if active_tags:
        st.info("Active tags: " + ", ".join(active_tags))
        if st.button("Clear active ETF tags"):
            st.session_state.etf_quick_tags = []
            st.rerun()

    etf_results = search_etf_catalog(etf_query, active_tags, selected_categories, etf_match_mode)
    st.markdown(f"### Results ({len(etf_results)} ETFs found)")
    if etf_results.empty:
        st.warning("No ETFs matched. Try fewer tags, switch from All to Any, or search a broader keyword.")
    else:
        st.dataframe(etf_results.head(max_etf_results), use_container_width=True, hide_index=True)

        # Optional: feed chosen ETF directly back into the main ticker field by showing copyable tickers.
        st.caption("Top matches: " + ", ".join(etf_results.head(10)["Ticker"].astype(str).tolist()))

    st.markdown("---")
    st.markdown("### 💸 Fee Saver")
    st.caption("Enter ETFs you hold. The tool estimates whether a cheaper similar ETF exists in the searchable catalog.")
