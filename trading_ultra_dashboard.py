...
with tab_sector:
    ...
    if scan_df.empty:
        ...
        sector = scan_df.groupby("Sector", as_index=False).agg(
            AvgBuy=("Buy", "mean"), AvgSell=("Sell", "mean"), AvgWhale=("WhaleScore", "mean"),
            AvgRSI=("RSI", "mean"), Count=("Ticker", "count")
        )
        sector["NetFlow"] = sector["AvgBuy"] - sector["AvgSell"]
        sector["AbsFlow"] = sector["NetFlow"].abs() + 1
        ...
        fig_tree = go.Figure(go.Treemap(
            ...
            parents=[""] * len(sector),
            values=sector["AbsFlow"],
            marker=dict(colors=sector["NetFlow"], colorscale="RdYlGn", cmid=0, colorbar=dict(title="Net Flow")),
            customdata=np.stack([sector["AvgBuy"].round(1), sector["AvgSell"].round(1), sector["AvgWhale"].round(1), sector["Count"]], axis=-1),
            hovertemplate="<b>%{label}</b><br>Net Flow: %{color:.1f}<br>Avg Buy: %{customdata[0]}<br>Avg Sell: %{customdata[1]}<br>Avg Whale: %{customdata[2]}<br>Tickers: %{customdata[3]}<extra></extra>"
        ))
        fig_tree.update_layout(height=520, title="Sector Rotation Heatmap — Green = Accumulation, Red = Distribution")
        st.plotly_chart(fig_tree, use_container_width=True)

        fig_bar = go.Figure(go.Bar(x=sector["Sector"], y=sector["NetFlow"], marker_color=sector["NetFlow"], marker_colorscale="RdYlGn"))
        fig_bar.update_layout(height=420, title="Sector Net Flow: Avg Buy Score - Avg Sell Score", yaxis_title="Net Flow")
        st.plotly_chart(fig_bar, use_container_width=True)

        st.write("### Top tickers by sector")
        for sec in sector["Sector"].head(8):
            sub = scan_df[scan_df["Sector"] == sec].sort_values("Buy", ascending=False).head(5)
